...
if(db.init(getServletContext().getRealPath("code/database.txt"))) {
    ...
    try {
	b = new String(request.getParameter("poll_id"));
	poll_id = Integer.parseInt(b);
    } catch(Exception e) {
        b = "";
    }
    ...
    if(a.length()>0 && b.length()>0 && d.length()>0 && poll_id>0 && conn!=null && request.getParameterValues("opt")!=null) {
	vote = new VotePoll(poll_id,conn);
...
<jsp:param name="poll_id" value="<%= poll_id %>" />
</jsp:forward>
<%
	    System.exit(0);
	} else {
    ...
    if(b.length()>0 && conn!=null && poll_id>0) {
	//Specified
        vote = new VotePoll(poll_id,conn);
