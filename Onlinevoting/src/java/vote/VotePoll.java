...
public class VotePoll {
	...
	try {
	    if(value>0) {
		//Load specified poll
		st = conn.prepareStatement("SELECT * FROM vote_polls WHERE poll_id=?");
		st.setInt(1, value);
	    } else {
		//Load last active poll
		st = conn.prepareStatement("SELECT * FROM vote_polls WHERE enabled='Y' AND expired='N' ORDER BY poll_id DESC LIMIT 1");	
	    }
		...
		try {
		    ...
		    description = new String(rs.getString("description").getBytes("ISO-8859-1"), "UTF-8");
		...
		//Load options	
		st = conn.prepareStatement("SELECT * FROM vote_votes WHERE poll_id=? ORDER BY option_id ASC");
		st.setInt(1, id);
		rs = st.executeQuery();
		options = new Vector<VoteOption>();
		total = 0;
		...
		while(rs.next()) {
		    try {
			t_str = new String(rs.getString("option_text").getBytes("ISO-8859-1"), "UTF-8");
		    } catch(UnsupportedEncodingException e) {
			t_str = rs.getString("option_text");
			e.printStackTrace();
    		    }
		    VoteOption op = new VoteOption(id,rs.getInt("option_id"),t_str,rs.getInt("counter"));
		    options.add(op);
	...
	try {
	    PreparedStatement st = conn.prepareStatement("UPDATE vote_polls SET enabled=? WHERE poll_id=?");
	...
	if(!checkValid()) return false;
	try {
	    PreparedStatement st = conn.prepareStatement("UPDATE vote_polls SET expired=? WHERE poll_id=?");
	    st.setString(1, value);
	    st.setInt(2, id);
	    int n = st.executeUpdate();
	    if(n>0) {
	...
	if(!checkValid()) return false;
	try {
	    PreparedStatement st = conn.prepareStatement("UPDATE vote_polls SET title=?,max_answers=?,description=?,enabled=?,anonymous=?,expired=? WHERE poll_id=?");
	    st.setString(1, title);
	    st.setInt(2, max_answers);
	    st.setString(3, description);
	    st.setString(4, enabled);
	    st.setString(5, anonymous);
	    st.setString(6, expired);
	    st.setInt(7, id);
	    int n = st.executeUpdate();
	    if(n>0) {
		status = "Poll is updated";
		...
		for(int i=0;i<options.size();i++) {
			...
			if(op.getOptionid()<1) {
			    ...
			    //Insert
			    st = conn.prepareStatement("INSERT INTO vote_votes VALUES(?,?,?,0)");
			    st.setInt(1, id);
			...
			if(op.getOptionid()<1) {
			    ...
			    //Update
			    st = conn.prepareStatement("UPDATE vote_votes SET option_text=? WHERE poll_id=? AND option_id=?");
			    st.setInt(2, id);
		...
		try {
		    ...
		    st = conn.prepareStatement("DELETE FROM vote_votes WHERE poll_id=? AND option_id NOT IN (" + cl + ")");
		    st.setInt(1, id);
		    st.executeUpdate();
		} catch(Exception e) {
		...
		if(np.size()>0) {
		    ...
		    st = conn.prepareStatement("SELECT MAX(option_id) AS ob FROM vote_votes WHERE poll_id=?");
		    st.setInt(1, id);
		    ResultSet rs = st.executeQuery();
		    int ob = 1;
		    if(rs.next()) {
	...
	if(id>0) {
	    try {
		PreparedStatement st = conn.prepareStatement("DELETE FROM vote_polls WHERE poll_id=?");
		st.setInt(1, id);
		st.executeUpdate();
		st.close();
	...
	try {
	    ...
	    PreparedStatement st = conn.prepareStatement("INSERT INTO vote_polls (title,max_answers,description,enabled,anonymous,expired) VALUES(?,?,?,?,?,?)");
	    st.setString(1, title);
	    st.setInt(2, max_answers);
	    st.setString(3, description);
	    st.setString(4, enabled);
	    ...
	    PreparedStatement sst = conn.prepareStatement("SELECT poll_id FROM vote_polls WHERE title=? AND max_answers=? AND description=? AND enabled=? AND anonymous=? AND expired=? ORDER BY poll_id DESC LIMIT 1");
	    sst.setString(1, title);
	    sst.setInt(2, max_answers);
	    sst.setString(3, description);
	    sst.setString(4, enabled);
	    sst.setString(5, anonymous);
	    sst.setString(6, expired);
	...
	try {
	    ...
	    sst = conn.prepareStatement("SELECT * FROM vote_polls WHERE poll_id=?");
	    sst.setInt(1, id);
	    ResultSet rs = sst.executeQuery();
	    if(rs.next()) {
		//check if expired
		...
		sst = conn.prepareStatement("SELECT ip AS user FROM vote_iplocks WHERE poll_id=? AND ip=?");
		ust = conn.prepareStatement("INSERT INTO vote_iplocks VALUES(?,?)");
		//Validate
		...
		//Process user voting
		sst = conn.prepareStatement("SELECT username AS user FROM vote_userlocks WHERE poll_id=? AND username=?");
		ust = conn.prepareStatement("INSERT INTO vote_userlocks VALUES(?,?)");
		//Validate
	    ...
	    sst = conn.prepareStatement("UPDATE vote_votes SET counter=counter+1 WHERE poll_id=? AND option_id=?");
	    sst.setInt(1, id);
	    for(int i=0;i<votes.length;i++) {
		sst.setInt(2, rv[i]);
		sst.executeUpdate();
